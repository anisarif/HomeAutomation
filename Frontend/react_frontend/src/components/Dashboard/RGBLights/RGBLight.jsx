...
const RGBLight = ({ name, id }) => {
    ...
    const send = (isOn, hex, pct) => {
        if (!isOn) { setRGBColor(id, 0, 0, 0); return; }
        const { r, g, b } = hexToRgb(hex);
        const f = pct / 100;
        setRGBColor(id, Math.round(r * f), Math.round(g * f), Math.round(b * f));
    };

    const handleToggle = () => {
        const next = !on;
        setOn(next);
        send(next, color, intensity);
    };

    const handleColor = (e) => {
        setColor(e.target.value);
        if (on) send(true, e.target.value, intensity);
    };

    const handleIntensity = (e) => {
        const val = parseInt(e.target.value);
        setIntensity(val);
        if (on) send(true, color, val);
    };
        ...
        <div className="flex flex-col gap-3 mx-4 py-3 border-b border-slate-300 last:border-0">
            ...
            <div className="flex items-center gap-4">
                <div
                    ...
                    style={{ backgroundColor: on ? color : '#9ca3af' }}
                />
                <input
                    type="color"
                    value={color}
